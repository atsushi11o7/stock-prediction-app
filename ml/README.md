# 株価予測システム

TOPIX Core30銘柄を対象とした、LSTM深層学習モデルによる12ヶ月先株価予測システム

---

## 📋 目次

- [概要](#概要)
- [クイックスタート](#クイックスタート)
- [システム構成](#システム構成)
- [運用ガイド](#運用ガイド)
  - [日次パイプライン](#日次パイプライン)
  - [月次パイプライン](#月次パイプライン)
- [開発者ガイド](#開発者ガイド)
  - [初回学習](#初回学習)
  - [推論](#推論)
  - [ファインチューニング](#ファインチューニング)
- [AWS Lambda デプロイメント](#aws-lambda-デプロイメント)
- [トラブルシューティング](#トラブルシューティング)
- [関連ドキュメント](#関連ドキュメント)

---

## 概要

### システムの特徴

- **データ取得**: yfinance API経由で日次・過去データを取得
- **特徴量**: 週次23特徴 × 156週 + 静的6特徴 + ポジション2特徴
- **モデル**: BiLSTM + セクターEmbedding + MLP（約664K パラメータ）
- **学習**: PyTorch Lightning、ONNX エクスポート
- **推論**: ONNX Runtime（CPU専用、Lambda対応）
- **出力**: 1~12ヶ月先の月次株価予測
- **運用**: 日次推論 + 月次ファインチューニング

### パフォーマンス

| 項目 | 値 |
|-----|---|
| 学習サンプル数 | 約40,000 |
| 学習時間（GPU） | 約20-30分 |
| 推論時間（31銘柄） | 約3-5秒 |
| ファインチューニング時間（CPU） | 約5-7分 |
| モデルサイズ（ONNX） | 約2.5MB |

### ディレクトリ構造

```
ml/
├── config/                    # 設定ファイル
│   ├── train.yaml             # 学習設定
│   ├── predict.yaml           # 推論設定
│   ├── finetune.yaml          # ファインチューニング設定
│   ├── enrich_universe.yaml   # 静的特徴量更新設定
│   ├── fetch_daily_universe.yaml   # 日次データ取得設定
│   ├── fetch_daily_backfill.yaml   # バックフィル設定
│   ├── sectors.yaml           # セクター定義（共通）
│   └── universes/             # Universe定義
│       └── enrich_*.yaml      # 静的特徴量付きUniverse
├── src/                       # ソースコード
│   ├── common/                # 共通モジュール
│   │   ├── logging_config.py  # ロギング設定
│   │   └── s3_operations.py   # S3操作
│   ├── data/                  # データ取得スクリプト
│   │   ├── utils/             # ユーティリティ
│   │   │   ├── config.py      # 設定読み込み（共通）
│   │   │   ├── universe_loader.py  # Universe読み込み
│   │   │   └── yfin.py        # yfinance操作
│   │   ├── fetch_daily_universe.py   # 日次データ取得
│   │   └── fetch_daily_backfill.py   # バックフィル
│   ├── features/              # 特徴量計算
│   │   ├── enrich_universe.py # 静的特徴量付与
│   │   ├── weekly_features.py # 週次特徴量
│   │   ├── valuation_features.py  # バリュエーション特徴量
│   │   └── position_features.py   # ポジション特徴量
│   ├── inference/             # 推論エンジン
│   │   └── predict.py         # ONNX推論
│   ├── model_src/             # モデル定義
│   │   ├── models/            # LSTMモデル
│   │   ├── lit_modules/       # PyTorch Lightningモジュール
│   │   └── datamodules/       # DataModule
│   ├── pipeline/              # パイプライン
│   │   ├── daily_pipeline.py  # 日次パイプライン
│   │   └── monthly_pipeline.py    # 月次パイプライン
│   └── training/              # 学習スクリプト
│       ├── train.py           # 初回学習
│       ├── finetune.py        # ファインチューニング
│       └── utils/             # 学習ユーティリティ
├── data/                      # データディレクトリ
│   ├── training/daily/        # 学習用日次データ
│   └── serving/daily/         # 推論用日次データ
├── docs/                      # ドキュメント
├── checkpoints/               # 学習チェックポイント
├── models/onnx/               # ONNXモデル
├── logs/                      # ログファイル
└── README.md                  # このファイル
```

---

## クイックスタート

### 初回セットアップ

```bash
# 1. 静的特徴量の付与
python src/features/enrich_universe.py --config config/enrich_universe.yaml

# 2. 過去データ取得（5年分）
python src/data/fetch_daily_backfill.py --config config/fetch_daily_backfill.yaml

# 3. 初回学習
python src/training/train.py --config config/train.yaml
```

### 日次運用（毎営業日）

```bash
# パイプライン実行（データ取得 + 推論）
python src/pipeline/daily_pipeline.py \
  --fetch-config config/fetch_daily_universe.yaml \
  --predict-config config/predict.yaml

# 単体で推論のみ実行
python src/inference/predict.py --config config/predict.yaml

# 単体でデータ取得のみ実行
python src/data/fetch_daily_universe.py --config config/fetch_daily_universe.yaml
```

### 月次運用（毎月1日）

```bash
# パイプライン実行（特徴量更新 + ファインチューニング）
python src/pipeline/monthly_pipeline.py \
  --enrich-config config/enrich_universe.yaml \
  --finetune-config config/finetune.yaml
```

---

## システム構成

### 運用フロー

```
┌─────────────────────────────────────────────────────────┐
│                   初回セットアップ                        │
│  1. Universe YAML作成                                   │
│  2. 静的特徴量付与 (enrich_universe.py)                  │
│  3. 過去データ取得 (fetch_daily_backfill.py)             │
│  4. 初回学習 (train.py)                                  │
└─────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
│              日次運用（毎営業日 JST 23:00）                │
│  1. 日次データ取得 (fetch_daily_universe.py)             │
│  2. 推論実行 (predict.py)                                │
│  3. 結果S3アップロード                                    │
└─────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
│              月次運用（毎月1日 JST 00:00）                 │
│  1. 静的特徴量更新 (enrich_universe.py)                  │
│  2. ファインチューニング (finetune.py)                    │
│  3. 新モデルS3アップロード                                │
└─────────────────────────────────────────────────────────┘
```

### モデルアーキテクチャ

```
入力:
  - weekly_seq: (156週, 23特徴)
  - static_features: (6特徴) ← PER, PBR, 配当利回り等
  - position_features: (2特徴) ← sin/cos(week in year)
  - sector_id: (1)

モデル:
  SectorEmbedding → BiLSTM → MLP → 出力

出力:
  - 1~12ヶ月先対数リターン (12値)
  - 各月の予測株価に変換して出力
```

---

## 運用ガイド

### 日次パイプライン

毎営業日実行する処理です。最新の市場データを取得し、12ヶ月先の株価を予測します。

#### 実行方法

```bash
# 設定ファイルベースで実行（推奨）
python src/pipeline/daily_pipeline.py \
  --fetch-config config/fetch_daily_universe.yaml \
  --predict-config config/predict.yaml

# データ取得をスキップ（推論のみ）
python src/pipeline/daily_pipeline.py \
  --predict-config config/predict.yaml \
  --skip-fetch

# S3環境で実行（設定ファイルでenv: "s3"を指定）
python src/pipeline/daily_pipeline.py \
  --fetch-config config/fetch_daily_universe.yaml \
  --predict-config config/predict.yaml
```

#### 処理フロー

```
[Step 1: 日次データ取得]
  - yfinance APIから最新のOHLCVデータを取得
  - JSON形式で保存
  ↓
[Step 2: 推論実行]
  - ONNXモデルをロード
  - 各銘柄について:
    - 週次特徴量を計算 (156週 × 23特徴)
    - 静的特徴量を計算 (6特徴)
    - ポジション特徴量を計算 (2特徴)
    - 12ヶ月先のリターンを予測
  - 結果をJSON形式で保存
```

#### 出力例

```json
{
  "timestamp": "2025-12-20T23:00:00",
  "num_predictions": 31,
  "predictions": [
    {
      "ticker": "2914.T",
      "as_of_date": "2025-12-20",
      "current_price": 4250.5,
      "predictions": [
        {"month": 1, "predicted_price": 4280.2, "return": 0.007},
        {"month": 2, "predicted_price": 4310.5, "return": 0.014},
        {"month": 3, "predicted_price": 4341.0, "return": 0.021},
        ...
        {"month": 12, "predicted_price": 4625.3, "return": 0.088}
      ],
      "predicted_12m_price": 4625.3,
      "predicted_12m_return": 0.088,
      "sector": "Consumer Defensive"
    }
  ]
}
```

#### パフォーマンス

- **処理時間**: 約5-10分
- **メモリ使用量**: 約200-300MB
- **Lambda推奨設定**: メモリ512MB、タイムアウト600秒

### 月次パイプライン

毎月1日に実行する処理です。静的特徴量を更新し、モデルをファインチューニングします。

#### 実行方法

```bash
# 設定ファイルベースで実行（推奨）
python src/pipeline/monthly_pipeline.py \
  --enrich-config config/enrich_universe.yaml \
  --finetune-config config/finetune.yaml

# 特徴量更新をスキップ
python src/pipeline/monthly_pipeline.py \
  --finetune-config config/finetune.yaml \
  --skip-enrich

# S3環境で実行（設定ファイルでenv: "s3"を指定）
python src/pipeline/monthly_pipeline.py \
  --enrich-config config/enrich_universe.yaml \
  --finetune-config config/finetune.yaml
```

#### 処理フロー

```
[Step 1: 静的特徴量更新]
  - yfinance APIから最新の静的情報を取得
    - PER (株価収益率)
    - PBR (株価純資産倍率)
    - DividendYield (配当利回り)
  ↓
[Step 2: ファインチューニング]
  - データサンプリング:
    - 直近1ヶ月: 約690サンプル (67%)
    - 過去データ: 約345サンプル (33%, ランダム)
  - モデル再構築
  - 少数エポックで学習 (過学習防止)
  - ONNX形式でエクスポート
```

#### パフォーマンス

- **処理時間**: 約10-15分
- **メモリ使用量**: 約1.5-2GB
- **Lambda推奨設定**: メモリ2048-3008MB、タイムアウト900秒

---

## 開発者ガイド

### 初回学習

初回の学習では、過去5年分のデータを使用してLSTMモデルを学習します。

#### データ準備

```bash
# 1. 静的特徴量付与
python src/features/enrich_universe.py --config config/enrich_universe.yaml

# 2. 過去データ取得（5年分）
python src/data/fetch_daily_backfill.py --config config/fetch_daily_backfill.yaml
```

#### 学習実行

```bash
# 基本実行
python src/training/train.py --config config/train.yaml
```

設定ファイルの詳細は [config/README.md](config/README.md) を参照してください。

#### 出力ファイル

```
checkpoints/
  └── best_model.ckpt           # ベストモデル（固定名、上書き更新）

models/onnx/
  ├── best_model.onnx           # ONNXモデル（固定名）
  └── best_model.json           # メタデータ（セクターマッピング等）
```

### 推論

学習済みONNXモデルを使用して、12ヶ月先の株価を予測します。

#### 実行方法

```bash
# 設定ファイルベースで実行
python src/inference/predict.py --config config/predict.yaml
```

設定ファイルの詳細は [config/README.md](config/README.md) を参照してください。

#### 推論の流れ

1. Universe YAMLから銘柄リストと静的情報を取得
2. 各銘柄の日次データを読み込み
3. 週次特徴量を計算（156週 × 23特徴）
4. 静的特徴量を計算（6特徴）
5. ポジション特徴量を計算（2特徴）
6. ONNX推論を実行
7. 結果をJSON形式で保存

#### 出力ファイル

```
predictions/
  ├── 2025-12-20.json    # 日付別ファイル（履歴用）
  ├── 2025-12-19.json
  ├── ...
  └── latest.json        # 最新の予測結果（バックエンドAPI参照用）
```

#### 予測結果の利用例

```python
import json

# 最新の結果を読み込み
with open("predictions/latest.json", "r") as f:
    results = json.load(f)

# または日付を指定して読み込み
# with open("predictions/2025-12-20.json", "r") as f:
#     results = json.load(f)

# 上位5銘柄をランキング
sorted_preds = sorted(
    results["predictions"],
    key=lambda x: x["predicted_12m_return"],
    reverse=True
)

print("Top 5 Predicted Returns (12M):")
for i, pred in enumerate(sorted_preds[:5], 1):
    print(f"{i}. {pred['ticker']}: {pred['predicted_12m_return']:+.2%}")
```

### ファインチューニング

学習済みモデルを月次でファインチューニングして、最新のトレンドに適応させます。

#### なぜファインチューニングが必要か

1. **市場環境の変化**: 株式市場は常に変化し、新しいパターンが出現
2. **モデルの鮮度維持**: 古いデータで学習したモデルは最新トレンドを捉えられない
3. **継続的改善**: 毎月新しいデータを追加することで予測精度が向上

#### 実行方法

```bash
# 設定ファイルベースで実行
python src/training/finetune.py --config config/finetune.yaml
```

設定ファイルの詳細は [config/README.md](config/README.md) を参照してください。

#### データサンプリング戦略

```
直近1ヶ月: 約1,200サンプル (67%)
過去データ: 約600サンプル (33%, ランダム)
─────────────────────────────
合計:      約1,800サンプル
```

過去データもランダムサンプリングすることで:
- 過学習を防止
- 長期パターンを保持
- 新しいトレンドと既存知識のバランスを取る

---

## AWS Lambda デプロイメント

### 日次パイプライン Lambda設定

#### 関数設定

```yaml
Function Name: stock-forecast-daily-pipeline
Runtime: Python 3.11
Memory: 512MB
Timeout: 600 seconds (10分)
Architecture: x86_64
```

#### 環境変数

```bash
CONFIG_PATH=/opt/config/predict.yaml
# 設定ファイル内で env: "s3" を指定
```

#### Lambda ハンドラー

```python
# lambda_handler.py
import sys
sys.path.insert(0, '/opt/ml/src')

from inference.predict import main

def lambda_handler(event, context):
    """AWS Lambda ハンドラー"""
    # 設定ファイルで env: "s3" が指定されていれば自動でS3を使用
    try:
        main()
        return {
            'statusCode': 200,
            'body': 'Daily pipeline completed successfully'
        }
    except Exception as e:
        return {
            'statusCode': 500,
            'body': f'Daily pipeline failed: {str(e)}'
        }
```

#### EventBridge スケジュール

```yaml
# 毎営業日 JST 23:00 (UTC 14:00)
Schedule: cron(0 14 ? * MON-FRI *)
```

### 月次パイプライン Lambda設定

#### 関数設定

```yaml
Function Name: stock-forecast-monthly-pipeline
Runtime: Python 3.11
Memory: 3008MB
Timeout: 900 seconds (15分)
Architecture: x86_64
```

#### 環境変数

```bash
CONFIG_PATH=/opt/config/finetune.yaml
# 設定ファイル内で env: "s3" を指定
```

#### EventBridge スケジュール

```yaml
# 毎月1日 JST 00:00 (UTC 15:00, 前日)
Schedule: cron(0 15 1 * ? *)
```

### Lambda Layerの作成

#### 依存ライブラリ

```bash
# requirements.txt
numpy==1.24.3
pandas==2.0.3
onnxruntime==1.16.0
pyyaml==6.0.1
yfinance==0.2.28
boto3==1.28.0
```

#### Layerビルド

```bash
mkdir -p lambda-layer/python
pip install -r requirements.txt -t lambda-layer/python/
cd lambda-layer
zip -r lambda-layer.zip python/
```

---

## トラブルシューティング

### 日次パイプライン

#### データ取得失敗

**症状**:
```
ERROR - Failed to fetch data for ticker: 2914.T
```

**原因と対処**:
1. yfinance API レート制限 → 待機時間を増やす
2. ネットワーク接続エラー → リトライ回数を増やす
3. ティッカーシンボルが無効 → Universe YAMLを確認

#### 推論失敗（データ不足）

**症状**:
```
ERROR - Insufficient data for 2914.T: only 100 weeks
```

**対処**:
```bash
# 過去データを取得（156週分必要）
python src/data/fetch_daily_backfill.py \
  --universe-yaml config/universes/enrich_topix_core_30_20251031.yaml \
  --period 5y
```

#### ONNXモデルが見つからない

**症状**:
```
ERROR - ONNX model not found: models/onnx/best_model.onnx
```

**対処**:
```bash
# 学習を実行してモデルを作成
python src/training/train.py --config config/train.yaml
```

### 月次パイプライン

#### メモリ不足

**症状**:
```
ERROR - Out of memory
```

**対処**:
- Lambda メモリ設定を増やす: 3008MB
- バッチサイズを減らす: `--batch-size 16`
- `num_workers=0` を確認（Lambda必須）

#### タイムアウト

**症状**:
```
Task timed out after 900.00 seconds
```

**対処**:
- エポック数を減らす: `--num-epochs 3`
- データサンプル数を減らす: `--recent-months 1`

### 一般的な問題

#### S3アップロード失敗

**症状**:
```
ERROR - Error uploading to S3: Access Denied
```

**対処**:
1. IAMロールの権限確認（S3 PutObject権限）
2. バケット名を確認: `--s3-bucket`
3. AWS認証情報を確認

#### CUDA out of memory（学習時）

**対処**:
```yaml
# config/train.yamlを編集
data:
  batch_size: 16  # 32 → 16

training:
  accumulate_grad_batches: 2  # 勾配蓄積
```

---

## 関連ドキュメント

### 設定ガイド

| ドキュメント | 目的 |
|------------|------|
| [config/README.md](config/README.md) | 設定ファイルガイド（env切り替え、各設定の詳細） |

### 運用ガイド（運用担当者向け）

| ドキュメント | 目的 |
|------------|------|
| [OPERATIONS_REFERENCE.md](docs/OPERATIONS_REFERENCE.md) | 運用リファレンス（詳細な運用手順） |

### 技術ドキュメント（開発者向け）

| ドキュメント | 目的 |
|------------|------|
| [stock_prediction_model_spec.md](docs/stock_prediction_model_spec.md) | モデル仕様書（入出力、アーキテクチャ） |
| [IMPLEMENTATION_SUMMARY.md](docs/IMPLEMENTATION_SUMMARY.md) | 実装サマリー |
